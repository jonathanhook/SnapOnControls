...
#define NDEF_DAS 0x000D
#define HEADER_EXTRA_LEN 0x4
#define PAYLOAD_EXTRA_LEN 0x4 
#define LAN_CODE "en"

#define NUM_DIGITAL_VALUES 1
#define NUM_ANALOG_VALUES 1
...
static void writeControlData(void)
{
	...
	for(i = 0; i < NUM_DIGITAL_VALUES; i++)
	{ 
		writeEEPROMByte(NDEF_DAS + i, digital[i]);
	}
	
		for(i = 0; i < NUM_DIGITAL_VALUES; i++)
	{ 
		writeEEPROMByte(NDEF_DAS + NUM_DIGITAL_VALUES + i, analog[i]);
	}
}                         
